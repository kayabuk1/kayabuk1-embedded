...
◇エラー9：カーブのきつい場所が曲がり切れず復帰動作が挟まる。
◇エラー10：復帰動作が挟まる時、後退⇒ごく僅かに首振り⇒再度後退⇒ごく僅かに首振りを繰り返してしまう。後退⇒ある程度首振りでコースをもう少し捉えたい。
...
エラー10：ほかの人のモータードライバー焼き切れ故障頻発
対処：モーターに0.1μFのコンデンサーをはんだ付け。またモータードライバーVm⇔GND間に470μF電解コンデンサを追加（急停止急発進時の安定出力＆ノイズ対策）
